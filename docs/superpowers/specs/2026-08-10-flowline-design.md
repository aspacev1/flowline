...
**Organization** — `id`, `name`, `slug` (уникален глобально).
...
**Project** — `id`, `org_id`, `name`, `slug` (уникален в пределах организации), `timezone`, `holidays` (список дат), `deadline` (целевая дата проекта, nullable), `shift_threshold_days` (по умолчанию 2), `plan_approved_at`, `plan_version` (целое, растёт при переутверждении).
...
**OrgLlmCredential** — `org_id`, `provider`, `encrypted_key`. Ключ шифруется симметрично секретом приложения, наружу не отдаётся никогда — только признак «ключ настроен».
...
Длительность задаётся в **рабочих днях**. Суббота и воскресенье считаются нерабочими всегда, дополнительные исключения — в `Project.holidays`. Вся работа с датами идёт через две функции: «прибавить N рабочих дней» и «сколько рабочих дней между датами». Прямых операций с календарными днями в коде нет.
...
4. **Календарь** — переходы через выходные и праздники, длительность в один день, длительность, начинающаяся в пятницу.
5. **Дедлайн** — задача, заканчивающаяся ровно в день дедлайна, не считается просроченной; окончание проекта берётся как максимум по всем задачам, включая созданные сверх плана.
6. **Языки** — полнота словарей: в `en` и `ru` нет ключей, отсутствующих в `az`, и наоборот; тест падает при рассинхроне. Отдельно — азербайджанская i-ловушка: `"I"` и `"İ"` в слагах, email и поиске приводятся к регистру одинаково независимо от локали процесса.
7. **AI** — на записанных ответах модели, без сети: валидная схема применяется, битая отбивается и не роняет сессию.
8. **Сквозной сценарий** — создать проект через интервью, утвердить план, сдвинуть задачу с причиной, открыть публичную ссылку в другом браузере, убедиться, что видны диаграмма и комментарии, но не внутренние заметки.
