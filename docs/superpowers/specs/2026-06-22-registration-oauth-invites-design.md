...
- `POST /auth/onboarding` — requires auth (the JWT cookie from the just-completed register/OAuth step). Body: `{ fullName, position, companyName?, inviteToken? }`.
  ...
  - Else: create a new `organizations` row with `name = companyName` (required in this branch), create `organization_members` row with role `owner`.
  - Updates `users.full_name` (existing column) and `users.position`.
- Frontend routes a freshly authenticated user with no org membership to the Onboarding screen automatically (checked via `GET /auth/me` returning `organization: null`).
