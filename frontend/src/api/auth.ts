...
export type User = {
  ...
  /**
   * Виден ли этому человеку пункт «Админ-панель». Решает список ADMIN_EMAILS
   * на сервере — здесь только чтобы не показывать пункт меню, ведущий
   * заведомо в отказ: доступ к самой панели сервер проверяет заново.
   */
  is_admin: boolean;
};
