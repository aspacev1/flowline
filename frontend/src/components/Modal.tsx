...
export function Modal({ title, onClose, dirty = false, children }: ModalProps) {
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      // Esc поверх самого вопроса означает «продолжить», а не «закрыть»:
      // клавиша отменяет последнее действие, а не доводит его до конца. Иначе
      // два Esc подряд — привычный жест «закрыть всё» — вели бы ровно к той
      // потере, ради которой вопрос и задан.
      if (asking) {
        dismiss();
        return;
      }
